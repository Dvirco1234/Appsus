...
function getGoogleBooks(txt) {
    const searchTerms = utilService.load(SEARCH_KEY) || {}
    if(searchTerms[txt]){
        console.log('get from cache');
        return Promise.resolve(searchTerms[txt])
    }
    const googleBooks = `https://www.googleapis.com/books/v1/volumes?printType=books&q=%20${txt}`
    return axios.get(googleBooks).then((res) => {
        console.log(res)
        ...
        searchTerms[txt] = books
        utilService.save(SEARCH_KEY, searchTerms)
        return books
    })
}
